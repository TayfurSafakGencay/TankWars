...
{
  public class HexagonObstacle : MonoBehaviour, IObstacle
  {
    [SerializeField]
    private GameObject hexagonObstacle;
    [SerializeField]
    ...
    public void Initialize()
    {
      ...
      for (int i = 0; i < floorCount; i++)
      {
        ...
        GameObject hexagonObstacleInstantiate = Instantiate(hexagonObstacle, position, rotation, transform);
        hexagonObstacleInstantiate.transform.localPosition = position;
        hexagonObstacleInstantiate.GetComponent<MeshRenderer>().material = newMaterial;
        position.y++;
